...
public:
  ...
  public:
    ...
    {
      return FK_solver::tissue_is_white(p) ? 0.6 : 0.3;
      // return 1;
    }
  };
  ...
  public:
    Tensor<2, dim> matrix_value(const Point<dim> & p /* , Tensor<2, dim> &values */ ) const
    {
      ...
      const double d_axn = FK_solver::tissue_is_white(p) ? 3.0 : 0.0;
      Tensor<1, dim> n = FK_solver::fiber_direction(p);

      Tensor<2, dim> values;
      for (unsigned int i = 0; i < dim; ++i)
        values[i][i] = d_ext;                      // 外扩散（各向同性部分）
  
      for (unsigned int i = 0; i < dim; ++i)
        for (unsigned int j = 0; j < dim; ++j)
          values[i][j] += d_axn * n[i] * n[j];     // 轴突方向扩散（各向异性部分）
