...
public:
  ...
  bool tissue_is_white(const Point<dim> &p)
  {
    // For testing: white matter is x > 0.5
    return p[0] > 0.5;
  }

  Tensor<1, dim> fiber_direction(const Point<dim> & /*p*/)
  {
    Tensor<1, dim> n;
  ...
  {
  public:
    Tensor<2, dim> matrix_value(const Point<dim> & /*p*/ /* ,Tensor<2,dim> &values */) const
    {
      Tensor<2, dim> values;
      ...
      // const double d_axn = tissue_is_white(p) ? 3.0 : 0.0;
      // Tensor<1, dim> n = fiber_direction(p);
